...
#define WorkspacePath \
  "/tmp/"  // make sure 1.it already exists; 2.program have permission(rwx) to
           // it;

// these are signals, which will cause programe to terminate
// remove signal from this set if you have new signal handler for it
int SIG_SET[] = {SIGHUP,    SIGINT,  SIGQUIT,   SIGILL,  SIGTRAP,
...
string run_cmd(const string &cmd) {
  ...
  char buf[1024] = {0};
  while (fgets(buf, sizeof(buf), pipe) != NULL) run_result += buf;
...
string get_remote_proj_path(const string &relative_header_path) {
  ...
  if (pos != string::npos) str.erase(pos, subStr.length());
...
void handle_exception() { print_backtrace(0); }

void segmentfault_test() {
  int *x = 0;
  *x = 0;
}

void throw_test() { throw 100; }

void default_beforecrash() {

}

void Register(void (*beforecrash)() = default_beforecrash) {
  struct sigaction sigact;

  sigact.sa_handler = print_backtrace;
  sigact.sa_flags = SA_RESTART | SA_SIGINFO;

  call_before_crash = beforecrash;
